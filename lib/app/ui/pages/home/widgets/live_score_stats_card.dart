...
class LivescoreStatsCard extends StatelessWidget {
  final List<Batter> batters;
  final List<Bowler> bowlers;

  const LivescoreStatsCard({
    super.key,
    this.batters = const [],
    this.bowlers = const [],
  });
  ...
  Widget _buildPartnershipInfo(BuildContext context) {
    return Padding(
      padding: EdgeInsets.only(
          left: 8.0.w, right: 16.0.w, top: 8.0.h, bottom: 8.0.h),
      child: Row(
        mainAxisSize: MainAxisSize.min,
        children: [
          _buildInfoText(
            context,
            "${Lang.of(context).lbl_pship} ${batters.isNotEmpty ? batters.first.pship : ''}",
          ),
          Spacer(),
          _buildInfoText(
            context,
            "${Lang.of(context).lbl_lastwkt} ${batters.isNotEmpty ? batters.first.lastwkt : ''}",
          ),
        ],
      ),
    );
  ...
  Widget _buildBowlerSection(BuildContext context) {
    return Column(
      children: [
        _buildBowlerHeader(context),
        Padding(
          padding: EdgeInsets.symmetric(horizontal: 8.0.w),
          child: const Divider(height: 0, thickness: 1.0),
        ),
        ...bowlers.map((bowler) => _BowlerRow(bowler: bowler)),
